...
def toggle_availability(product_id: str, payload: dict, _: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    restaurant_id = _.get('restaurant_id')
    ...
    product = (
        db.query(Product)
        .filter(and_(Product.id == product_id, Product.restaurant_id == restaurant_id))
        .first()
    )
